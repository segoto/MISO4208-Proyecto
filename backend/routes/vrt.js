...
router.get('/hab_movil', function (req, res, next) {
    response = [];
    try{
        child_process.execFile('C:\\Users\\teo-1\\Desktop\\MISO4208-Proyecto\\backend\\utils\\vrt_Habitica.cmd', {shell:process.env.ComSpec},function(error, stdout, stderr) {
            //console.log(stdout);
            //console.log(error);
            //console.log("hola");
            for(let i=1;i<6;i++){
                resemble('./utils/base_images/habitica_img'+i+'.png')
                    .compareTo('./utils/mod_images/habitica_img'+i+'.png')
                    .ignoreColors()
                    .onComplete(function(data) {
                        fs.writeFile('./utils/diff_images/habitica_img'+i+'.png', data.getBuffer(), (err) => {
                            if (err) throw err;
                            // console.log('The file has been saved!');
                        });
                        data['id'] = 'img'+i
                        data['imagen_diff'] = `http://localhost:3001/diff_images/habitica_img${i}.png`;
                        data['imagen_mod'] = `http://localhost:3001/mod_images/habitica_img${i}.png`;
                        data['imagen_base'] = `http://localhost:3001/base_images/habitica_img${i}.png`;
                        response.push(data);
                    });
            }
            res.end(JSON.stringify(response));
