...
const uri = "mongodb+srv://" + databaseUser + ":" + databasePassword + "@educapp-viylh.gcp.mongodb.net/" + databaseName + "?retryWrites=true&w=majority";
...
router.get('/e2e', function (req, res, next) {
  ...
  MongoClient.connect(uri,
      function(err, client) {
        ...
        db.collection('tests').find({type: 'e2e'}).toArray(function(err, result) {
          if (err) {
            res.send('Error');
          }
          res.send(result);
...
router.get('/random', function (req, res, next) {
  ...
  MongoClient.connect(uri,
      function(err, client) {
        ...
        let db = client.db('testing');
        db.collection('tests').find({type: 'random'}).toArray(function(err, result) {
          if (err) {
            res.send('Error');
          }
          res.send(result);
...
router.get('/tests', function (req, res, next) {
  ...
  MongoClient.connect(uri,
      function(err, client) {
        ...
        let db = client.db('testing');
        db.collection('tests').find().toArray(function(err, result) {
          if (err) {
            res.send('Error');
          }
          res.send(result);
